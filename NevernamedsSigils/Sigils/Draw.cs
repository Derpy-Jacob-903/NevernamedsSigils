...
namespace NevernamedsSigils
{
    public class Draw : AbilityBehaviour
    {
        public static void Init()
        {
            ...
            AbilityInfo newSigil = SigilSetupUtility.MakeNewSigil("Draw Large Bloon", "When [creature] is played, its owner draws cards equal to the number inscribed on this sigil.",
                      typeof(Draw),
        ...
        public CardInfo DrawLargeBloon(Deck deck, CardInfo specificCard)
        {
            CardInfo cardInfo;
            if (specificCard != null && deck.cards.Contains(specificCard))
            {
            ...
            else
            {
                ...
                List<CardInfo> list = deck.cards;
                foreach (CardInfo i in list) 
                {
                    if (i.tribes.Contains(Tribe.Squirrel))
                    {
                ...
                int num = deck.randomSeed;
                deck.randomSeed = num + 1;
                cardInfo = list[SeededRandom.Range(min, count, num)];
            }
            deck.cards.Remove(cardInfo);
            return cardInfo;
        }
    }
}