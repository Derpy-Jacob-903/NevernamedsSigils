...
{
    public abstract class AltLatch : Latch
    {
        public static void Init_() //
        {
            AbilityInfo newSigil = SigilSetupUtility.MakeNewSigil("Alt Latch", "An abstract Latch that allows effects other that adding a sigil by overriding ForLatched(CardSlot selectedSlot)",
                      typeof(SlowLatch),
            ...
            ability_ = newSigil.ability;
        }
        public static Ability ability_;
        ...
        public override IEnumerator OnPreDeathAnimation(bool wasSacrifice)
        {
            ...
            if (validTargets.Count > 0)
            {
                ...
                else
                {
                    ...
                    {
                        loc1.selectedSlot = s;
                    }, new Action<CardSlot>(this.OnInvalidTarget), delegate (CardSlot s)
                    {
                        if (s.Card != null)
                        {
                            loc1.cardAnim.AimWeaponAnim(s.transform.position);
                        }
    ...
    [HarmonyPatch]
    public class Act1AltLatchAbilityFix
    {
        ...
        private static IEnumerator Postfix(IEnumerator enumerator, AltLatch __state, bool wasSacrifice)
        {
            ...
            if (selectedSlot != null && selectedSlot.Card != null)
            {
                ...
                yield return new WaitForSeconds(0.75f); 
                yield return __state.LearnAbility(0f); //here?
            }
        }
    }
}